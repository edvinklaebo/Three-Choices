...
namespace Core.Abilities
{
    ...
    ///     Holds both the balance config (damage, Weak stacks/duration, cooldown) and the
    ...
    {
        [Header("Shadow Bolt")]
        [Tooltip("Number of Weak stacks applied on hit.")]
        [Min(1)] [SerializeField] private int _weakStacks = 1;
        [Tooltip("Number of turns the Weak effect lasts.")]
        ...
        public void EditorInit(string identifier, string soName,
                               int baseDamage = 8, int damagePerUpgrade = 2, int cooldownRounds = 0,
                               int weakStacks = 1, int weakDuration = 2)
        {
            id = identifier;
            displayName = soName;
